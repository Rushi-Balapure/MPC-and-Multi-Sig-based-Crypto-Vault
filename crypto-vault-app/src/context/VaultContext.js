...
export const VaultProvider = ({ children }) => {
  ...
  }, [token, authUser?.email, fetchAllTeams, checkAndRefreshSession]);

  // Load personal assets with authentication
  const loadPersonalAssets = useCallback(async (userId) => {
    if (!userId) return;
  ...
  // Load pending invitations with authentication
  const loadPendingInvitations = useCallback(async (email) => {
    if (!email) return;
  ...
  // Fetch team transactions function
  const fetchTeamTransactions = useCallback(async (teamId) => {
    if (!teamId || !token) return;
    
    try {
      const response = await fetch(`${API_BASE_URL}/api/teams/${teamId}/transactions`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        console.log('⚠️ Team transactions endpoint not ready');
        setTransactionHistory([]);
      }
    } catch (err) {
      console.error('❌ Error fetching team transactions:', err);
      setError('Failed to fetch team transactions');
      setTransactionHistory([]);
    }
  }, [token]);
  ...
  }, [isLoggedIn, authUser, loadUserData]);
